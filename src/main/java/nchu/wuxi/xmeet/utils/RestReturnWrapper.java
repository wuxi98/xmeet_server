            ...
            if (body instanceof Resource) {
                ...
                try {
                    if (RestReturn.isRestReturnJson((String) body)) {
                        ...
                        System.out.println("情况4 普通的返回，需要统一格式，把数据赋值回去即可。");

                        return restReturn.success(body, "",0);
                    }
                } catch (Exception e) {
                    // 因为 API返回值为String，理论上不会走到这个分支。
                    return restReturn.error(10000, body, e.getMessage(),0);
                }
            } else {
                ...
                if(body instanceof RestReturn){
                    ...
                    return body;
                }else if(body instanceof StringReturn){
                    RestReturn restReturn = new RestReturn();
                    return restReturn.success(body, ((StringReturn) body).getData().toString(),0);
                } else{
                    //情况6 非字符串非统一格式的返回，需要统一格式
                    //需要判定是否是抛出的异常返回（统一到错误输出）
//                    System.out.println("情况6 非字符串非统一格式的返回，需要统一格式");
                    RestReturn restReturn = new RestReturn();
                    return restReturn.success(body, "",0);
                }
            }
